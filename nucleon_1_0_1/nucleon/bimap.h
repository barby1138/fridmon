 ...
 *
 * Changes in v1.2:
 *   · integrated in the Nucleon Library
 *   
 * Changes in v1.3:
 *   · ported to VC++ 7.1, note: 'from' and 'to' interfaces changed!
 *
 * Last modified: November 20th, 2005
 */
...
private:
  ...
  typedef std::set<
    ...
    p_compare<
      from_type_,
      from_compare>,
    BIMAP_REBIND_9B698EF9_C6E9_4BC4_A7D2_5B4D71155761(allocator_type_,const from_type_*)>
                                            from_set;
  typedef std::set<
    const to_type_*,
    p_compare<
      to_type_,
      to_compare>,
    BIMAP_REBIND_9B698EF9_C6E9_4BC4_A7D2_5B4D71155761(allocator_type_,const to_type_*)>
                                            to_set;
  ...
  static element * element_by_from(const from_type_* pf)
  {
    return
      reinterpret_cast<element*>(
        reinterpret_cast<char*>(
          const_cast<from_type_ *>(pf))-
            BIMAP_OFFSETOF_9B698EF9_C6E9_4BC4_A7D2_5B4D71155761(element,first));
  }

  static element * element_by_to(const to_type_* pt)
  {
    return
      reinterpret_cast<element *>(
        reinterpret_cast<char*>(
          const_cast<to_type_ *>(pt))-
...
public:
  ...
  public:
    ...
    class iterator:public std::iterator<std::bidirectional_iterator_tag,const value_type>
    {
      ...
      friend class prebimap<from_type_,to_type_,from_compare,to_compare,allocator_type>;
    ...
    friend void swap(from_impl& x,from_impl& y)
    {
      x.swap(y);
    }
...
#else
  FRIEND_CLASS from_impl::iterator;
  FRIEND_CLASS from_impl::const_iterator;
#endif
  ...
  public:
    ...
    friend void swap(to_impl& x,to_impl& y)
    {
      x.swap(y);
    }
...
#else
  FRIEND_CLASS to_impl::iterator;
  FRIEND_CLASS to_impl::const_iterator;
#endif
    ...
    const typename from_impl::value_type& x)
  {
    if(!from.adjacent(fit.fit,x.first)){
      ...
      if(fit!=fset.end()){ /* small chance x is already inserted */
        ...
        if(tnit!=tset.end()&&element_by_from(*(fit.fit))==element_by_to(*tnit)){
          return std::make_pair(fit,to_impl::iterator(tnit));
        }
        else throw duplicate_value();
      }
    }
    ...
    return std::make_pair(from_impl::iterator(fnit),to_impl::iterator(tnit));
  }
...
protected:
  prebimap(
    ...
    allocator(al),
    fset(p_compare<from_type_,from_compare>(from_comp),fset_allocator_type(allocator)),
    tset(p_compare<to_type_,to_compare>(to_comp),tset_allocator_type(allocator))
  {}
...
{
  typedef prebimap<from_type_,to_type_,from_compare,to_compare,allocator_type> type; 
};
#endif
  ...
  public prebimap_identity<from_type_,to_type_,from_compare,to_compare,allocator_type_>::type
{
  ...
  typedef typename prebimap_identity<from_type_,to_type_,from_compare,to_compare,allocator_type_>::type super;
#else
  public prebimap<from_type_,to_type_,from_compare,to_compare,allocator_type_>
{
  ...
  typedef typename super::to_impl::value_type                             to_value_type;
  typedef typename super::to_impl::iterator                               to_iterator;

public:
  typedef typename  super::from_impl::key_type               key_type;
  typedef typename  super::from_impl::mapped_type            mapped_type;
  typedef typename  super::from_impl::referent_type          referent_type;
  typedef typename  super::from_impl::data_type              data_type;
  typedef typename  super::from_impl::key_compare            key_compare;
  typedef typename  super::from_impl::allocator_type         allocator_type;
  typedef typename  super::from_impl::value_type             value_type;
  typedef typename  super::from_impl::value_compare          value_compare;
  typedef typename  super::from_impl::size_type              size_type;
  typedef typename  super::from_impl::difference_type        difference_type;
  typedef typename  super::from_impl::pointer                pointer;
  typedef typename  super::from_impl::const_pointer          const_pointer;
  typedef typename  super::from_impl::reference              reference;
  typedef typename  super::from_impl::const_reference        const_reference;
  typedef typename  super::from_impl::iterator               iterator;
  typedef typename  super::from_impl::const_iterator         const_iterator;
  typedef typename  super::from_impl::reverse_iterator       reverse_iterator;
  typedef typename  super::from_impl::const_reverse_iterator const_reverse_iterator;

  iterator                 begin(){return super::from.begin();}
  const_iterator           begin()const{return super::from.begin();}
  iterator                 end(){return super::from.end();}
  const_iterator           end()const{return super::from.end();}
  reverse_iterator         rbegin(){return super::from.rbegin();}
  const_reverse_iterator   rbegin()const{return super::from.rbegin();}
  reverse_iterator         rend(){return super::from.rend();}
  const_reverse_iterator   rend()const{return super::from.rend();}
  size_type                size()const{return super::from.size();}
  size_type                max_size()const{return super::from.max_size();}
  bool                     empty()const{return super::from.empty();}
  allocator_type           get_allocator()const{return super::from.get_allocator();}
  from_binding_t           operator[](const key_type& key){return super::from[key];}
  const_from_binding_t     operator[](const key_type& key)const{return super::from[key];}
  using                    super::insert;
  std::pair<iterator,bool> insert(const value_type& x){return super::from.insert(x);}
  iterator                 insert(iterator it,const value_type& x){return super::from.insert(it,x);}
  template<
    typename it_type>
  void                     insert(it_type first,it_type last){super::from.insert(first,last);}

#ifdef _MSC_VER /* see note in from::erase */
  iterator                 erase(iterator it){return super::from.erase(it);}
#else
  void                     erase(iterator it){super::from.erase(it);}
#endif

  void                     erase(iterator first,iterator last){super::from.erase(first,last);}
  size_type                erase(const key_type& key){return super::from.erase(key);}
  void                     clear(){super::from.clear();}
  void                     swap(bimap_equal_types& x){super::from.swap(x.super::from);}
  friend void              swap(bimap_equal_types& x,bimap_equal_types& y){x.swap(y);}
  key_compare              key_comp()const{return super::from.key_comp();}
  value_compare            value_comp()const{return super::from.value_comp();}
  iterator                 find(const key_type& key){return super::from.find(key);}
  const_iterator           find(const key_type& key)const{return super::from.find(key);}
  size_type                count(const key_type& key)const{return super::from.count(key);}
  iterator                 lower_bound(const key_type& key){return super::from.lower_bound(key);}
  const_iterator           lower_bound(const key_type& key)const{return super::from.lower_bound(key);}
  iterator                 upper_bound(const key_type& key){return super::from.upper_bound(key);}
  const_iterator           upper_bound(const key_type& key)const{return super::from.upper_bound(key);}
  std::pair<
    iterator,
    iterator>              equal_range(const key_type& key){return super::from.equal_range(key);}
  std::pair<
    const_iterator,
    const_iterator>        equal_range(const key_type& key)const{return super::from.equal_range(key);}

  /* Promotion of unambiguous parts of the to memberspace */

  std::pair<
    to_iterator,
    bool>                  insert(const to_value_type& x){return super::to.insert(x);}
  to_iterator              insert(to_iterator it,const to_value_type& x){return super::to.insert(it,x);}
  void                     insert(const to_value_type *first,const to_value_type *last)
                           {super::to.insert(first,last);}

#ifdef _MSC_VER /* see note in from::erase */
  to_iterator              erase(to_iterator it){return super::to.erase(it);}
#else
  void                     erase(to_iterator it){super::to.erase(it);}
#endif

  void                     erase(to_iterator first,to_iterator last){super::to.erase(first,last);}
  ...
  public prebimap<from_type_,to_type_,from_compare,to_compare,allocator_type_>
{
  ...
  typedef typename super::to_impl::key_type                               to_key_type;
  typedef typename super::to_impl::value_type                             to_value_type;
  typedef typename super::to_impl::iterator                               to_iterator;
  typedef typename super::to_impl::const_iterator                         const_to_iterator;
...
public:
  ...
  size_type                erase(const key_type& key){return super::from.erase(key);}
  void                     clear(){super::from.clear();}
  void                     swap(bimap_different_types& x){super::from.swap(x.super::from);}
  ...
  to_binding_t             operator[](const to_key_type& key){return super::to[key];}
  const_to_binding_t       operator[](const to_key_type& key)const{return super::to[key];}
  std::pair<
    to_iterator,
    bool>                  insert(const to_value_type& x){return super::to.insert(x);}
  to_iterator              insert(to_iterator it,const to_value_type& x){return super::to.insert(it,x);}
  ...
  void                     erase(to_iterator first,to_iterator last){super::to.erase(first,last);}
  size_type                erase(const to_key_type& key){return super::to.erase(key);}
  to_iterator              find(const to_key_type& key){return super::to.find(key);}
  const_to_iterator        find(const to_key_type& key)const{return super::to.find(key);}
  size_type                count(const to_key_type& key)const{return super::to.count(key);}
  to_iterator              lower_bound(const to_key_type& key){return super::to.lower_bound(key);}
  const_to_iterator        lower_bound(const to_key_type& key)const{return super::to.lower_bound(key);}
  to_iterator              upper_bound(const to_key_type& key){return super::to.upper_bound(key);}
  const_to_iterator        upper_bound(const to_key_type& key)const{return super::to.upper_bound(key);}
  std::pair<
    to_iterator,
    to_iterator>           equal_range(const to_key_type& key){return super::to.equal_range(key);}
  std::pair<
    const_to_iterator,
    const_to_iterator>     equal_range(const to_key_type& key)const{return super::to.equal_range(key);}
...
<
  ...
  typename allocator_type = quark::allocator<direct_pair<const from_type_, const to_type_>, quark::user_mempool>
>
class bimap:
  public 
    bimap_detail::select<
 ...
 * right *after* the insertion point, in disagreement with the standard.
 */
      typename super::from_impl::iterator fhint=from.end();
      typename super::to_impl::iterator   thint=to.end();
      for(typename inv_bimap::to_impl::const_iterator it=r.to.begin();it!=r.to.end();++it){
        insert(fhint,thint,*it);
      }
#else
      typename super::from_impl::iterator fhint=super::from.end();
      typename super::to_impl::iterator   thint=super::to.end();
      for(typename inv_bimap::to_impl::const_iterator it=r.super::to.begin();it!=r.super::to.end();++it){
        std::pair<typename super::from_impl::iterator,typename super::to_impl::iterator>
          p=insert(fhint,thint,*it);
        fhint=p.first;
        thint=p.second;
      }
 ...
 */
  ...
  bool operator==(const bimap& r)const{return super::from==r.super::from;}
  bool operator!=(const bimap& r)const{return super::from!=r.super::from;}
  bool operator< (const bimap& r)const{return super::from<r.super::from;}
  bool operator> (const bimap& r)const{return super::from>r.super::from;}
  bool operator<=(const bimap& r)const{return super::from<=r.super::from;}
