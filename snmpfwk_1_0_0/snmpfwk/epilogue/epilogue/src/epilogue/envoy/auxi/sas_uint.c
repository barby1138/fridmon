/* $Header: /services/cvsroot/fridmon/snmpfwk_1_0_0/snmpfwk/epilogue/epilogue/src/epilogue/envoy/auxi/sas_uint.c,v 1.1 2006/06/29 12:17:26 smasyutin Exp $ */
 ...
 *
 * Revision 7.2  1997/03/20 06:50:25  sra
...
void
  undo_uint_async(OIDC_T         lastmatch,
                  int            compc,
                  OIDC_T        *compl,
                  SNMP_PKT_T    *pktp,
                  VB_T          *vbp)
{
*(UINT_32_T *)(VB_TO_COOKIE(vbp)) = (UINT_32_T)vbp->vb_priv;
undoproc_good(pktp, vbp);
return;
}

/* Set a 32-bit unsigned integer referenced by cookie, and save the old value
   in case we need to undo the set.  */
void
  set_uint_async(OIDC_T          lastmatch,
                 int             compc,
                 OIDC_T         *compl,
                 SNMP_PKT_T     *pktp,
                 VB_T           *vbp)
...
vbp->vb_priv = (PTR_T)(*(UINT_32_T *)(VB_TO_COOKIE(vbp)));

*(UINT_32_T *)(VB_TO_COOKIE(vbp)) = VB_GET_UINT32(vbp);

undoproc_set(pktp, vbp, undo_uint_async);
